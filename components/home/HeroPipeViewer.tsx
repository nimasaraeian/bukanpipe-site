// @ts-nocheck -- vendored verbatim from the approved standalone viewer; it is
// plain DOM script, not site code, and is kept unedited on purpose.
/* eslint-disable */
"use client";
...
/*
 * The pipe viewer, exactly as it was built and approved as a standalone page:
 * its own stylesheet, its own markup and its own script, moved across without
 * being re-designed for the site. Four lines differ and none of them are
 * visible — the page loaded three.js r128 from a CDN, the site bundles r186,
 * where colour management and two encoding flags moved.
 *
 * It is deliberately NOT refactored into the site's components. It is meant to
 * stay the thing that was signed off, so changes to it are changes someone
 * asked for rather than drift.
 */

const CSS = String.raw`
.pipe-artifact{
  color-scheme:light;
  --ink:#0E1D33; --ink-2:#37475F; --muted:#77839A;
  --paper:#F4F3EF; --card:#FCFBF8; --line:#DFDCD3;
  --water:#0A64F5; --gas:#D9A41E; --hair:rgba(14,29,51,.10);
  --stage-1:#EEEFF1; --stage-2:#D6DAE1;
}
@media (prefers-color-scheme:dark){
  :root:not([data-theme="light"]) .pipe-artifact{
    color-scheme:dark;
    --ink:#EAF0F8; --ink-2:#B3C2D6; --muted:#7D8CA3;
    --paper:#0A1524; --card:#10203552; --line:rgba(255,255,255,.12);
    --hair:rgba(255,255,255,.10);
    --stage-1:#122238; --stage-2:#070F1C;
  }
}
:root[data-theme="dark"] .pipe-artifact{
  color-scheme:dark;
  --ink:#EAF0F8; --ink-2:#B3C2D6; --muted:#7D8CA3;
  --paper:#0A1524; --card:#10203552; --line:rgba(255,255,255,.12);
  --hair:rgba(255,255,255,.10);
...
.pipe-artifact{
  background:var(--paper);color:var(--ink);direction:rtl;
  font-family:"Vazirmatn",Tahoma,system-ui,sans-serif;
  font-size:15px;line-height:1.7;
}
.shell{max-width:1040px;margin-inline:auto;padding-inline:16px;padding-block:22px 40px}
.brand{display:flex;align-items:baseline;gap:10px;flex-wrap:wrap}
.brand b{font-weight:800;font-size:13px;letter-spacing:.06em;color:var(--water)}
.brand span{font-size:12.5px;color:var(--muted)}
h1{margin:10px 0 0;font-size:clamp(22px,4.4vw,30px);line-height:1.35;font-weight:800;text-wrap:balance}
.lede{margin:8px 0 0;max-width:62ch;color:var(--ink-2);font-size:14.5px}

/* ---- stage ---- */
.stage{
  position:relative;margin-top:20px;border:1px solid var(--line);border-radius:14px;
  background:radial-gradient(120% 100% at 50% 22%,var(--stage-1),var(--stage-2));
  height:min(56vh,420px);min-height:280px;overflow:hidden;touch-action:none;
}
.stage canvas{display:block;width:100%;height:100%;cursor:grab;outline:none}
.stage canvas:active{cursor:grabbing}
.hint{
  position:absolute;inset-inline-start:14px;bottom:12px;display:flex;align-items:center;gap:7px;
  font-size:12px;color:var(--muted);background:var(--card);border:1px solid var(--hair);
  border-radius:99px;padding:5px 12px;pointer-events:none;
  transition:opacity .5s ease;backdrop-filter:blur(6px)
}
.hint.gone{opacity:0}
.fallback{
  position:absolute;inset:0;display:none;place-items:center;text-align:center;
  padding:24px;color:var(--ink-2);font-size:14px
}
.stage.nogl .fallback{display:grid}
.stage.nogl canvas,.stage.nogl .hint{display:none}

/* ---- controls ---- */
.rail{margin-top:18px;display:grid;gap:14px}
.row{display:grid;gap:8px}
.row>h2{
  margin:0;font-size:11.5px;font-weight:600;letter-spacing:.04em;color:var(--muted)
}
.chips{display:flex;gap:6px;overflow-x:auto;padding-bottom:4px;scrollbar-width:thin}
.chip{
  flex:none;font:inherit;font-size:13px;font-variant-numeric:tabular-nums;
  padding:6px 13px;border-radius:8px;border:1px solid var(--line);
  background:var(--card);color:var(--ink-2);cursor:pointer;
  transition:border-color .15s,color .15s,background .15s
}
.chip:hover:not(:disabled){border-color:var(--water);color:var(--ink)}
.chip[aria-pressed="true"]{background:var(--water);border-color:var(--water);color:#fff;font-weight:600}
.chip:disabled{opacity:.3;cursor:not-allowed}
.chip:focus-visible{outline:2px solid var(--water);outline-offset:2px}
...
.spec{background:var(--paper);padding:13px 14px;display:grid;gap:3px}
...
.spec dd small{font-size:12px;font-weight:400;color:var(--muted);font-family:"Vazirmatn",Tahoma,sans-serif}
.spec dd.pair{display:flex;flex-wrap:wrap;gap:4px 12px;font-size:17px}
.spec dd.pair>span{unicode-bidi:isolate;display:inline-flex;align-items:baseline;gap:5px}
.spec.na dd{color:var(--muted)}
.note{margin-top:16px;font-size:12.5px;color:var(--muted);max-width:70ch}
.note b{color:var(--ink-2);font-weight:600}
@media (prefers-reduced-motion:reduce){*{transition:none!important}}
`;
const MARKUP = String.raw`<div class="shell">
  <div class="brand"><b>بوکان پایپ</b><span>نمونه آزمایشی — نمای سه‌بعدی محصول</span></div>
  <h1>لوله پلی اتیلن را بچرخانید و مقطعش را ببینید</h1>
  <p class="lede">با کشیدن ماوس یا انگشت، لوله در هر جهتی می‌چرخد. قطر و کلاس فشار را که عوض کنید، ضخامت دیواره روی همین مدل تغییر می‌کند — عددها از جدول ابعاد خود کارخانه می‌آیند، نه از تخمین.</p>

  <div class="stage" id="stage">
    <div class="hint" id="hint">برای چرخاندن بکشید</div>
    <p class="fallback">مرورگر شما WebGL را پشتیبانی نمی‌کند، بنابراین مدل سه‌بعدی نمایش داده نمی‌شود. مشخصات پایین همچنان درست است.</p>
  </div>

  <div class="rail">
    <div class="row">
      <h2>قطر خارجی اسمی — DN (میلی‌متر)</h2>
      <div class="chips" id="dnRow" role="group" aria-label="انتخاب قطر"></div>
    </div>
    <div class="row">
      <h2>کلاس فشار — SDR</h2>
      <div class="chips" id="sdrRow" role="group" aria-label="انتخاب کلاس فشار"></div>
    </div>
  </div>

  <dl class="specs" id="specs">
    <div class="spec"><dt>قطر خارجی</dt><dd id="oOd">—</dd></div>
    <div class="spec"><dt>ضخامت دیواره</dt><dd id="oWall">—</dd></div>
    <div class="spec"><dt>قطر داخلی</dt><dd id="oId">—</dd></div>
    <div class="spec"><dt>فشار اسمی</dt><dd id="oPn" class="pair">—</dd></div>
  </dl>

  <p class="note"><b>این نمونه با عکس ساخته نشده.</b> لوله یک مدل سه‌بعدی واقعی است که در مرورگر ساخته می‌شود، بنابراین از هر زاویه‌ای چرخانده می‌شود و نسبت ضخامت دیواره به قطر، همان نسبت واقعی جدول است. خطوط آبی نشانه لوله آبرسانی است.</p>
</div>`;

export function HeroPipeViewer() {
  ...
  useEffect(() => {
    ...
    import("three")
      ...
      .catch(() => {
        /* the page's own fallback copy stands in */
      });
  ...
  return (
    <section className="pipe-artifact" aria-label="نمای سه‌بعدی لوله پلی اتیلن">
