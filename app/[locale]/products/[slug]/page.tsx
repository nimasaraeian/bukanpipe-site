...
/** Unknown product slugs must 404 at the framework level (see lib/routing/ssg-config.ts). */
...
export async function generateStaticParams() {
  return ["fa", "en"].flatMap((locale) =>
    contentSlugsForKind(locale as Locale, "product").map((slug) => ({ locale, slug })),
  );
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const doc = getContentBySlug(localeParam, "product", slug);
  if (!doc) return {};
