...
describe("createRootMetadata", () => {
  it("exposes square PNG favicons Google Search can read above 48px", () => {
    ...
    expect(icons.icon?.[0]).toMatchObject({ url: "/bukan-pipe-icon-gray.png", sizes: "96x96", type: "image/png" });
    expect(icons.icon?.some((icon) => icon.url === "/bukan-pipe-icon-gray.png")).toBe(true);
